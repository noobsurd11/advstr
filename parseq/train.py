...
def main(config: DictConfig):
    ...
    if config.pretrained is not None:
        ...
        try:
            ...
            w = get_pretrained_weights(config.pretrained)
        except Exception:
            # Otherwise: treat as raw state_dict path
            print(f"[INFO] Loading raw state_dict from {config.pretrained}")
            sd = torch.load(config.pretrained, map_location="cpu")
            if "state_dict" in sd:
                sd = sd["state_dict"]
            w = sd
