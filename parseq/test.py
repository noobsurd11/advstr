...
# # If the user passes a custom LMDB dataset path
# # Detect custom weather LMDB (Option-2)
#     import os

# # Detect user passed a specific LMDB folder
#     if os.path.basename(args.data_root) in ["hazy", "snowy", "rainy"]:
#         dataset_name = os.path.basename(args.data_root)
#         args.data_root = os.path.dirname(args.data_root)  # strip the LMDB folder
#         manual_mode = True
#         test_set = [dataset_name]
#         print("Using custom LMDB dataset:", dataset_name)
#     else:
#         manual_mode = False
...
def main():
    ...
    # 🆕 NEW ARG: name of custom LMDB under <data_root>/test (e.g. hazy)
    parser.add_argument(
        '--custom_set',
        type=str,
        default=None,
        help="Name of custom LMDB under <data_root>/test (e.g. 'hazy', 'rainy', 'snowy')",
    )
    ...
    datamodule = SceneTextDataModule(
        args.data_root,          # e.g. parseq/data
        '_unused_',
        hp.img_size,
        hp.max_label_length,
        hp.charset_train,
    ...
    # 🧠 Decide which test sets to evaluate
    if args.custom_set is not None:
        # Custom LMDB: e.g. parseq/data/test/hazy
        test_set = [args.custom_set]
        manual_mode = True
        print(f"Using custom LMDB dataset: {args.custom_set}")
    else:
        # Original PARSeq behavior
        manual_mode = False
        test_set = list(SceneTextDataModule.TEST_BENCHMARK_SUB + SceneTextDataModule.TEST_BENCHMARK)
        if args.new:
            test_set += list(SceneTextDataModule.TEST_NEW)
        test_set = sorted(set(test_set))
    ...
    with open(args.checkpoint + '.log.txt', 'w') as f:
        for out in [f, sys.stdout]:
            if manual_mode:
                print(f'Custom set: {args.custom_set}', file=out)
                print_results_table([results[args.custom_set]], out)
            else:
                result_groups = {
                    'Benchmark (Subset)': SceneTextDataModule.TEST_BENCHMARK_SUB,
                    'Benchmark': SceneTextDataModule.TEST_BENCHMARK,
